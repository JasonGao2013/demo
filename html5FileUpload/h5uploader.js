...
$.fn.html5uploader = function(opts){
	...
	var defaults = {
		...
		itemTemplate:'<li id="${fileID}file"><div class="progress"><div class="progressbar"></div></div><span class="filename">${fileName}</span>&nbsp;&nbsp;&nbsp;&nbsp;<span class="progressnum">0/${fileSize}</span></li>',//上传队列显示的模板,最外层标签使用<li>
		onUploadStart:function(){},//上传开始时的动作
		onUploadSuccess:function(){},//上传成功的动作
		onUploadComplete:function(file, data){},//上传完成的动作
		...
		//创建文件对象
			  ...
			  onSelect: option.onSelect||function(files){
				 ...
				 for(var i=0;i<files.length;i++){
					...
					//判断是否是自动上传
					 if(option.auto){
						 FILE.funUploadFile(file);
						 }
				 }
				 
				 //如果配置非自动上传，绑定上传事件
//				 if(!option.auto){
//					_this.find('.uploadbtn').die().on('click',function(){
//						var index = parseInt($(this).parents('li').attr('id'));
//						FILE.funUploadFile(getFile(index,files));
//						});
//				 }
				 //为删除文件按钮绑定删除文件事件
			  ...
			  onProgress: function(file, loaded, total) {
				  var eleProgress = $('#'+file.index+'file .progress'), percent = (loaded / total * 100).toFixed(2) + '%';
...
//						  console.log(file.name);
						  xhr.setRequestHeader("FILENAME", encodeURI(file.name)); // nginx不能识别X_FILENAME 这种头信息
						  xhr.send(file);
					  }	
				  })(file);	
